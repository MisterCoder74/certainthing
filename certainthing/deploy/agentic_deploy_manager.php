<?php
/**
 * CertainThing – Agentic Deploy Manager
 * Location: ./deploy/agentic_deploy_manager.php
 * Users file: ../../data/users.json (relative to this file)
 *
 * Same authenticated file-browsing/editing/deletion/link-sharing engine as
 * deploy_manager.php, scoped one directory deeper: deploy/agentic/{user_id}/
 * instead of deploy/{user_id}/. That is where api/agentic_deploy.php (the
 * Deploy button inside an Agentic Workspace sandbox) writes each workspace's
 * deployed copy, one subfolder per workspace id — kept as a separate tree so
 * agentic sandbox deploys never collide with the normal chat deploy.php output.
 */
session_start();
...
if (
    ...
    $baseDir = __DIR__ . '/agentic/' . $userId;
...
if (isset($_GET['logout'])) {
    ...
    header('Location: agentic_deploy_manager.php');
    exit;
}
...
<head>
    ...
    <title>✦ CertainThing – Agentic Deploy Manager</title>
    <style>
/* ── Reset & Base ────────────────────────────────────────────────────────── */
...
<div class="dm-login-wrapper">
    <div class="dm-login-card">
        <h2>Agentic Deploy Manager</h2>
        <p class="sub">Sign in to manage your Agentic Workspace deployments</p>
        <?php if ($loginError): ?>
...
<!-- ═══════════════════════════════════════════════════════════════════════ -->
<header class="dm-header">
    <div class="dm-logo"><span class="icon">✦</span> CertainThing – Agentic Deploys</div>
    <div class="dm-header-right">
        <a href="../index.php" class="dm-back-link">← Back to CertainThing</a>
        <a href="../agentic_workspace.php" class="dm-back-link">🗂 Agentic Workspace</a>
        <a href="deploy_manager.php" class="dm-back-link">📦 Regular Deploys</a>
        <span class="dm-user-email"><?= htmlspecialchars($userEmail) ?></span>
        <a href="agentic_deploy_manager.php?logout=1" class="btn btn-sm">Logout</a>
    </div>
</header>
...
<div class="dm-main">
    ...
    <nav class="dm-breadcrumbs">
        <a href="agentic_deploy_manager.php">🏠 Agentic Deployments</a>
        <?php
        $cumulative = '';
        ...
        ?>
            ...
            <?php if ($i < count($pathParts) - 1): ?>
                <a href="agentic_deploy_manager.php?path=<?= urlencode($cumulative) ?>"><?= htmlspecialchars($part) ?></a>
            <?php else: ?>
    ...
    <?php if (empty($entries)): ?>
        <div class="dm-empty">
            <div class="icon">📁</div>
            <p><?= $currentPath ? 'This folder is empty.' : 'No agentic deployments yet.' ?></p>
        </div>
    <?php else: ?>
                ...
                <tr>
                    <td>
                        <div class="fname">
                            ...
                            <?php if ($entry['is_dir']): ?>
                                <a href="agentic_deploy_manager.php?path=<?= urlencode($entry['rel_path']) ?>">
                        ...
                        <div class="factions">
                            <?php if (!$entry['is_dir']): ?>
                                ...
                                <a class="btn-icon" title="View in browser"
                                   href="<?= htmlspecialchars($baseUrl . '/agentic/' . $userId . '/' . $entry['rel_path']) ?>"
                                   target="_blank">🔗</a>
                                <button class="btn-icon" title="Download"
                                    onclick="location.href='agentic_deploy_manager.php?action=download&file=<?= urlencode($entry['rel_path']) ?>'">⬇️</button>
                                ...
                                <button class="btn-icon" title="Copy link"
                                    onclick="copyLink('<?= htmlspecialchars($baseUrl . '/agentic/' . $userId . '/' . $entry['rel_path'], ENT_QUOTES) ?>')">📋</button>
...
function downloadZip(folder) {
    ...
    location.href = 'agentic_deploy_manager.php?action=download_zip&folder=' + encodeURIComponent(folder);
}
...
function deleteItem(path, isDir) {
    ...
    fetch('agentic_deploy_manager.php?action=delete_item', {
        method: 'POST',
...
function renameItem(path, currentName) {
    ...
    fetch('agentic_deploy_manager.php?action=rename_file', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
...
function openEditor(path) {
    ...
    fetch('agentic_deploy_manager.php?action=get_content&file=' + encodeURIComponent(path))
        .then(r => r.json())
        .then(d => {
...
function saveFile() {
    ...
    fetch('agentic_deploy_manager.php?action=save_file', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
            file: editorFilePath,
...
function createFile() {
    ...
    fetch('agentic_deploy_manager.php?action=create_file', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
